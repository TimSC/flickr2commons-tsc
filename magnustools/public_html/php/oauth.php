...
//		$this->params = parse_ini_file ( $this->ini_file ) ;
		$this->gUserAgent = 'flickr2commons-ng.toolforge.org' ;
//		if ( !isset($this->gUserAgent) or $this->gUserAgent == '' ) throw new Exception ( "Cannot get user agent from ini file '{$this->ini_file}'" ) ;
		$this->gConsumerKey = getenv( 'MW_CONSUMER_TOKEN' );
		if ( !isset($this->gConsumerKey) or $this->gConsumerKey == '' ) throw new Exception ( "Cannot get consumer key from env" ) ;
		$this->gConsumerSecret = getenv( 'MW_CONSUMER_SECRET' );
		if ( !isset($this->gConsumerSecret) or $this->gConsumerSecret == '' ) throw new Exception ( "Cannot get consumer secret from env" ) ;
	}
